...
- (void)czh_setView {
    ...
    toolBar.backgroundColor = [UIColor whiteColor];//CZHColor(0xf6f6f6);
    [containView addSubview:toolBar];
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, toolBar.frame.size.height - .5, toolBar.frame.size.width, .5)];
    
    line.backgroundColor = [UIColor colorWithRed:240.f/255.f green:240.f/255.f blue:240.f/255.f alpha:1.0f];
    ...
    UIButton *sureButton = [UIButton czh_buttonWithTarget:self action:@selector(buttonClick:) frame:CGRectMake(toolBar.czh_width - TOOLBAR_BUTTON_WIDTH, 0, TOOLBAR_BUTTON_WIDTH, toolBar.czh_height) titleColor:CZHColor(0xF76F6F) titleFont:CZHGlobelNormalFont(14) title:@"确定"];
